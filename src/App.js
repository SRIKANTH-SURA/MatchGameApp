...
class App extends Component {
  state = {
    ...
    matchedImgId: imagesList[0].id,
  }

  componentDidMount() {
    this.timerID = setInterval(this.timerElapsedSec, 1000)
  }
  ...
  onSelectMatchedImage = id => {
    const {matchedImgId} = this.state
    // const matchedImageDetails = imagesList.find(
    //   eachImage => eachImage.id === id,
    // )
    if (matchedImgId === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        matchedImgId: this.getRandomImgUrl(),
      }))
    } else {
      this.setState(prevState => ({isGameOver: !prevState.isGameOver}))
      clearInterval(this.timerID)
    ...
    return (
      <div className="score-card">
        <div className="scorecard-content">
          ...
          <h1 className="scorecard-score">{score}</h1>
          <button
            type="button"
  ...
  getMatchedImgUrl = () => {
    const {matchedImgId} = this.state
    const matchedDetails = imagesList.find(
      imgDetails => imgDetails.id === matchedImgId,
    )
    return matchedDetails.imageUrl
  }
  ...
  renderAppContent = () => {
    ...
    const matchedImgUrl = this.getMatchedImgUrl()

    return (
      <div className="app-content">
        <div className="match-img-container">
          <img className="match-img" src={matchedImgUrl} alt="match" />
        </div>
        ...
        <ul className="images-container">
          {filteredImages.map(imageDetails => (
            <ImageItem
              ...
              onSelectMatchedImage={this.onSelectMatchedImage}
            />
          ))}
        </ul>
      </div>
  ...
  render() {
    ...
    return (
      <div className="match-game-app">
        ...
        {isGameOver ? this.renderScoreCard() : this.renderAppContent()}
      </div>
    )
  }
}
